<?php
    ...
    if (!empty($_POST)){
        ...
        //If the eventname or eventdate is empty when the user submits
        ...
        if (empty($_POST['eventname']) || empty($_POST['eventdate'])) {
            ...
            $response["success"] = 0;
            $response["message"] = "Please Enter Both a Event Name and Date.";
        ...
        $query        = " SELECT 1 FROM event WHERE name = :event";
        ...
        if ($row) {
            ...
            $response["success"] = 0;
            $response["message"] = "I'm sorry, this event name is already in use";
            die(json_encode($response));
        }
        ...
        $query = "INSERT INTO event ( name, date ) VALUES ( :event, :date ) ";
